...
function PluginInspect(options = {}) {
  ...
  function configureServer(server) {
    ...
    server.printUrls = () => {
      const colorUrl = (url2) => kolorist.green(url2.replace(/:(\d+)\//, (_, port) => `:${kolorist.bold(port)}/`));
  ...
  async function generateBuild() {
    const targetDir = node_path.join(config.root, outputDir);
    ...
    await fs.writeFile(
      ...
      (await fs.readFile(node_path.join(targetDir, "index.html"), "utf-8")).replace(
    ...
    await dumpModuleInfo(node_path.join(reportsDir, "transform"), transformMap);
    await dumpModuleInfo(node_path.join(reportsDir, "transform-ssr"), transformMapSSR, true);
    return targetDir;
  }
  const plugin = {
    name: NAME,
